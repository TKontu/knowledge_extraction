...
def main():
    ...
    with Session(engine) as session:
        ...
        for ext in exts:
            ...
            for fname, fdata in ext.data.items():
                ...
                if isinstance(fdata, dict) and "quote" in fdata:
                    ...
                    items.extend(
                        x for x in fdata if isinstance(x, dict) and "quote" in x
                    )
        ...
        print(
            f"  Has char_offset:     {has_offset} ({has_offset / total_quotes * 100:.1f}%)"
        )
        print(
            f"  Null char_offset:    {null_offset} ({null_offset / total_quotes * 100:.1f}%)"
        )
        print(
            f"  No location at all:  {no_location} ({no_location / total_quotes * 100:.1f}%)"
        )
        ...
        if by_type:
            ...
            for etype in sorted(by_type.keys()):
                ...
                print(
                    f"  {etype:30s}  {nulls}/{total} null ({nulls / total * 100:.1f}%)"
                )
