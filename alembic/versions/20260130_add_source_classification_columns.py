...
def upgrade() -> None:
    ...
    op.add_column("sources", sa.Column("page_type", sa.String(50), nullable=True))
    op.add_column(
        "sources",
        sa.Column(
    ...
    op.add_column(
        "sources", sa.Column("classification_method", sa.String(20), nullable=True)
    )
    op.add_column(
        "sources", sa.Column("classification_confidence", sa.Float(), nullable=True)
    )

    # Index for filtering by page type within a project
    op.create_index("ix_sources_page_type", "sources", ["project_id", "page_type"])
