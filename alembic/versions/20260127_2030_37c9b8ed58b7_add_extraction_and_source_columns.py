...
def upgrade() -> None:
    ...
    op.add_column(
        "extractions",
        sa.Column(
            "entities_extracted", sa.Boolean(), nullable=True, server_default="false"
        ),
    )

    # Add created_by_job_id to sources table
    op.add_column("sources", sa.Column("created_by_job_id", UUID(), nullable=True))
    ...
    op.create_index("idx_sources_created_by_job_id", "sources", ["created_by_job_id"])
